...
do {
    let fullName = try foo.getFullName()
    fullName
} catch is Person.Errors { //matching specific type of error
    "Got an error"
}
...
// the following syntax should never be used as the app wil close
...
// this do and catch prove that the first error is only caught
do {
    try dog.barkAndRun()
        dog
} catch Dog.BarkingErrors.cannotBarkIsSleeping{
